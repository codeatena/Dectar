...
- (void)makeToast:(NSString *)message {
    [self makeToast:JJLocalizedString(message, nil)  duration:CSToastDefaultDuration position:nil];
}
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    ...
    // default to bottom
    return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - ((toast.frame.size.height / 2))-45) - CSToastVerticalPadding);
}
