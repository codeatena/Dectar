...
- (void)inviteUser:(XMPPJID *)jid withMessage:(NSString *)inviteMessageStr
{
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		NSXMLElement *invit = [NSXMLElement elementWithName:@"invite"];
		[invit addAttributeWithName:@"to" stringValue:[jid full]];
		
		if ([inviteMessageStr length] > 0)
		{
			[invit addChild:[NSXMLElement elementWithName:@"reason" stringValue:inviteMessageStr]];
		}
