...
//static const int kRedBannerColor = 0xF64747;
...
- (void)applyStyle
{
    [self putShadow:self.textLabel];
    [self putShadow:self];
    if (self.style == AFMInfoBannerStyleError) {
        [self setBackgroundColor:self.errorBackgroundColor ?:[UIColor whiteColor]];//UIColorFromRGB(kRedBannerColor)];
        [self.textLabel setTextColor:self.errorTextColor ?: [UIColor blackColor]];              //UIColorFromRGB(kDefaultTextColor)];
    ...
    [self.textLabel setFont:self.font ?:[UIFont fontWithName:@"Avenir-Book " size:18.0f]]; //[UIFont boldSystemFontOfSize:kFontSize]];
}
-(void)putShadow:(UIView*)View
{
...
- (void)setupViewsAndFrames
{
    ...
    UIViewController *topmostVC = [hierarchy topmostViewController];
    ...
    if (possibleBar && !possibleBar.translucent) {
        ...
        UINavigationController *navVC = [hierarchy topmostNavigationController];
