...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions xy:(CGPoint)point size:(CGSize)size isMultiple:(BOOL)isMultiple
{
    ...
    if (self = [super initWithFrame:rect])
    {
        ...
        if (isMultipleSelection) {
            ...
            NSInteger xOrigin=182;
            if(IS_IPHONE_6){
                xOrigin=153;
            }else if (IS_IPHONE_6P){
                xOrigin=133;
            }
            [btnDone setFrame:CGRectMake(rect.origin.x+xOrigin,rect.origin.y-93, 82, 31)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([self.arryData containsObject:indexPath]){
        imgarrow.frame=CGRectMake(230,5, 20, 20);
        imgarrow.image=[UIImage imageNamed:@"check_mark"];
	} else
        imgarrow.image=nil;
...
- (void)drawRect:(CGRect)rect {
  ...
  //  CGContextSetShadowWithColor(ctx, CGSizeMake(1, 1), 0.5f, [UIColor darkGrayColor].CGColor);
  //  [[UIColor colorWithWhite:1 alpha:1.] setFill];
    
    if (([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0)) {
        UIFont *font = [UIFont boldSystemFontOfSize:16];
        UIColor *cl=[UIColor darkGrayColor];
        ...
        [_kTitleText drawInRect:titleRect withFont:[UIFont boldSystemFontOfSize:16]];
